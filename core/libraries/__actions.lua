function library()
    ...
    function internal:new(bp)
        ...
        -- Class Functions.
        function self:getRanges() return ranges end
        function self:getDelays() return delays end

        function self:getRange(action)
        ...
        function self:buildAction(category, param)
        ...
        function self:getActionType(action)
        ...
        function self:getActionDelay(action)
        ...
        function self:perform(target, param, action, x, y, z)
        ...
        function self:acceptRaise()
            self:perform(bp.player, 0, 'accept raise')
        end

        function self:engage(target)
            self:perform(target, 0, 'engage')
        end

        function self:disengage()
        ...
        function self:engageAll(target)
        ...
        function self:switchAll(target)
        ...
        function self:item(item, target) -- UPDATE NEEDED!
    
        end

        function self:equip(name, slot) -- UPDATE NEEDED!
    
        end

        function self:tradeGil(npc, amount)
            local amount = tonumber(amount)
    
            if bp and npc and amount and npc.id and npc.index and amount ~= nil and amount <= windower.ffxi.get_items().gil then
        ...
        function self:synthesis(crystal, ingredients, materials)
            --[[
        ...
        function self:getOutgoingRotation()
        ...
        function self:keyCombo(combo, delay, wait)
    ...
    return setmetatable({}, {__call = internal})
